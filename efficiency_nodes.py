...
def encode_prompts(
    ...
    if return_type in ["base", "both"]:
        ...
        positive_encoded = bnk_adv_encode.AdvancedCLIPTextEncode().encode(
            clip, positive_prompt, token_normalization, weight_interpretation
        )[0]
        negative_encoded = bnk_adv_encode.AdvancedCLIPTextEncode().encode(
            clip, negative_prompt, token_normalization, weight_interpretation
        )[0]

    # Process refiner encodings if needed
    if (
        return_type in ["refiner", "both"]
        and is_sdxl
        and refiner_clip
        and refiner_clip_skip
        and ascore
    ):
        refiner_clip = CLIPSetLastLayer().set_last_layer(
            refiner_clip, refiner_clip_skip
        )[0]

        refiner_positive_encoded = bnk_adv_encode.AdvancedCLIPTextEncode().encode(
            refiner_clip, positive_prompt, token_normalization, weight_interpretation
        )[0]
        refiner_positive_encoded = bnk_adv_encode.AddCLIPSDXLRParams().encode(
            refiner_positive_encoded, empty_latent_width, empty_latent_height, ascore[0]
        )[0]

        refiner_negative_encoded = bnk_adv_encode.AdvancedCLIPTextEncode().encode(
            refiner_clip, negative_prompt, token_normalization, weight_interpretation
        )[0]
        refiner_negative_encoded = bnk_adv_encode.AddCLIPSDXLRParams().encode(
            refiner_negative_encoded, empty_latent_width, empty_latent_height, ascore[1]
        )[0]
...
class TSC_EfficientLoader:
    ...
    def efficientloader(
        ...
        # Create Empty Latent
        latent = torch.zeros(
            [batch_size, 4, empty_latent_height // 8, empty_latent_width // 8]
        ).cpu()
        ...
        if lora_name != "None" or lora_stack:
            ...
            model, clip, vae = load_checkpoint(
                ckpt_name, my_unique_id, cache=ckpt_cache, cache_overwrite=True
            )
            lora_params = None
...
class TSC_EfficientLoaderSDXL(TSC_EfficientLoader):
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "empty_latent_width": (
                    "INT",
                    {"default": 1024, "min": 64, "max": MAX_RESOLUTION, "step": 64},
                ),
                "empty_latent_height": (
                    "INT",
                    {"default": 1024, "min": 64, "max": MAX_RESOLUTION, "step": 64},
    ...
    def efficientloaderSDXL(
        ...
        return super().efficientloader(
            ...
            refiner_name=refiner_ckpt_name,
            ascore=(positive_ascore, negative_ascore),
            prompt=prompt,
...
class TSC_Unpack_SDXL_Tuple:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {"sdxl_tuple": ("SDXL_TUPLE",)},
        }
...
class TSC_LoRA_Stacker:
    ...
    def lora_stacker(self, input_mode, lora_count, lora_stack=None, **kwargs):
        ...
        if input_mode == "simple":
            ...
            loras = [
                (lora_name, lora_weight, lora_weight)
                for lora_name, lora_weight in zip(loras, weights)
                if lora_name != "None"
            ]
        else:
            model_strs = [
                kwargs.get(f"model_str_{i}") for i in range(1, lora_count + 1)
            ]
            clip_strs = [kwargs.get(f"clip_str_{i}") for i in range(1, lora_count + 1)]
            loras = [
                (lora_name, model_str, clip_str)
                for lora_name, model_str, clip_str in zip(loras, model_strs, clip_strs)
                if lora_name != "None"
            ]
...
class TSC_Control_Net_Stacker:
    ...
    def control_net_stacker(
        self, control_net, image, strength, start_percent, end_percent, cnet_stack=None
    ):
...
class TSC_KSampler:
    ...
    def sample(
        ...
        def vae_decode_latent(vae, samples, vae_decode):
            return (
                VAEDecodeTiled().decode(vae, samples, 320)[0]
                if "tiled" in vae_decode
                else VAEDecode().decode(vae, samples)[0]
            )

        def vae_encode_image(vae, pixels, vae_decode):
            return (
                VAEEncodeTiled().encode(vae, pixels, 320, overlap=64)[0]
                if "tiled" in vae_decode
                else VAEEncode().encode(vae, pixels)[0]
            )

        # ---------------------------------------------------------------------------------------------------------------
        def process_latent_image(
            model,
            seed,
            steps,
            cfg,
            ...
            def calculate_sigmas(model_sampling, scheduler_name: str, steps):
                if scheduler_name.startswith("AYS"):
                    return AlignYourStepsScheduler().get_sigmas(
                        scheduler_name.split(" ")[1], steps, denoise=1.0
                    )[0]
            ...
            try:
                ...
                if keys_exist_in_script("noise"):
                    rng_source, cfg_denoiser, add_seed_noise, m_seed, m_weight = script[
                        "noise"
                    ]
                    smZ_rng_source.rng_rand_source(
                        rng_source
                    ...
                    if add_seed_noise:
                        comfy.sample.prepare_noise = (
                            cg_mixed_seed_noise.get_mixed_noise_function(
                                comfy.sample.prepare_noise, m_seed, m_weight
                            )
                        )
                    else:
                        m_seed = m_weight = None
                else:
                    rng_source = cfg_denoiser = add_seed_noise = m_seed = m_weight = (
                        None
                    )
                ...
                if keys_exist_in_script("anim"):
                    ...
                    model = AnimateDiffLoaderWithContext().load_mm_and_inject_params(
                ...
                refiner_positive_hash = (
                    tensor_to_hash(refiner_positive[0][0])
                    if refiner_positive is not None
                    else None
                )
                refiner_negative_hash = (
                    tensor_to_hash(refiner_negative[0][0])
                ...
                model_identifier = (
                    [original_model_str, motion_model, beta_schedule, context_options]
                    if keys_exist_in_script("anim")
                    else [original_model_str]
                )

                parameters = [model_identifier] + [
                    seed,
                    steps,
                    cfg,
                    sampler_name,
                ...
                parameters = [
                    str(item) if not isinstance(item, type(latent_image_hash)) else item
                ...
                if keys_exist_in_script("hiresfix"):
                    ...
                    if latent_upscale_model is None:
                        ...
                        latent_upscale_model = (
                            AnimateDiffLoaderWithContext().load_mm_and_inject_params(
                                latent_upscale_model,
                                motion_model,
                                beta_schedule,
                                context_options,
                    ...
                    if hires_control_net is not None:
                        ...
                        if cnet_imgs is True:
                            cnet_imgs = load_ksampler_results(
                                "cnet_img", my_unique_id, [preprocessor]
                            )
                        ...
                        if cnet_imgs is None:
                            ...
                            cnet_imgs = AIO_Preprocessor().execute(
                                preprocessor, images
                            )[0]
                            store_ksampler_results(
                                "cnet_img", my_unique_id, cnet_imgs, [preprocessor]
                            )
                        positive = ControlNetApply().apply_controlnet(
                            positive, hires_control_net, cnet_imgs, hires_cnet_strength
                        )[0]

                    # Iterate for the given number of iterations
                    if upscale_type == "latent":
                        for _ in range(iterations):
                            upscaled_latent_image = latent_upscale_function().upscale(
                                samples, latent_upscaler, upscale_by
                            )[0]
                        ...
                        if images is None:
                            images = vae_decode_latent(vae, samples, vae_decode)
                            store_ksampler_results("image", my_unique_id, images)
                        images = ImageUpscaleWithModel().upscale(
                            pixel_upscale_model, images
                        )[0]
                        images = ImageScaleBy().upscale(
                            images, "nearest-exact", upscale_by / 4
                        )[0]
                ...
                if keys_exist_in_script("tile"):
                    ...
                    if images is None:
                        images = vae_decode_latent(vae, samples, vae_decode)
                        if not any(
                            keys_exist_in_script(key) for key in ["xyplot", "hiresfix"]
                        ):
                            store_ksampler_results("image", my_unique_id, images)

                    # Upscale image
                    upscaled_image = ImageScaleBy().upscale(
                        images, "nearest-exact", upscale_by
                    )[0]
                    ...
                    if tile_controlnet is not None:
                        positive = ControlNetApply().apply_controlnet(
                            positive, tile_controlnet, upscaled_image, 1
                        )[0]

                    # Sample latent
                    TSampler = bnk_tiled_samplers.TiledKSampler
                    samples = TSampler().sample(
                        model,
                        tile_seed,
                        tile_size,
                        tile_size,
                ...
                if keys_exist_in_script("anim"):
                    if images is None:
                        images = vae_decode_latent(vae, samples, vae_decode)
                        if not any(
                            keys_exist_in_script(key)
                            for key in ["xyplot", "hiresfix", "tile"]
                        ):
                            store_ksampler_results("image", my_unique_id, images)
                    gifs = AnimateDiffCombine().generate_gif(
                        images,
                        frame_rate,
                        loop_count,
                        format=format,
                ...
                if "true" in vae_decode:
                    if images is None:
                        ...
                        if all(
                            not keys_exist_in_script(key)
                            for key in ["xyplot", "hiresfix", "tile", "anim"]
                        ):
                            store_ksampler_results("image", my_unique_id, images)
                ...
                if cnet_imgs is not None and not True:
                    if preprocessor_imgs and upscale_type == "latent":
                        if keys_exist_in_script("xyplot"):
                            ...
                            if (
                                images.shape[1:3] != cnet_imgs.shape[1:3]
                ...
                elif preview_method == "none" or (
                    preview_method == "vae_decoded_only" and vae_decode == "false"
                ):
                    preview = {"images": list()}
        ...
        # If XY Plot
            ...
            if "noise_mask" in latent_image:
                ...
                latent_tensors = [
                    {"samples": img, "noise_mask": mask}
                    for img, mask in zip(samples_tensors, noise_mask_tensors)
                ]
            else:
            ...
            def suggest_ksampler(X_type, Y_type, current_sampler):
                for sampler, details in samplers.items():
                    if (
                        ...
                        and X_type not in details["disallowed"]
                        and Y_type not in details["disallowed"]
                    ):
                        return details["name"]
            ...
            def process_xy_for_print(value, replacement, type_):
                ...
                if type_ == "Seeds++ Batch" and isinstance(value, list):
                ...
                elif (type_ == "Checkpoint" or type_ == "Refiner") and isinstance(
                    value, list
                ):
                    # For each tuple in the list, return only the first value if the second or third value is None
                    return [
                        (
                            (os.path.basename(v[0]),) + v[1:]
                            if v[1] is None or v[2] is None
                            else (
                                (os.path.basename(v[0]), v[1])
                                if v[2] is None
                                else (os.path.basename(v[0]),) + v[1:]
                            )
                        )
                        for v in value
                    ]

                elif (type_ == "LoRA" or type_ == "LoRA Stacks") and isinstance(
                    value, list
                ):
                    # Return only the first Tuple of each inner array
                    return [
                        (
                            [(os.path.basename(v[0][0]),) + v[0][1:], "..."]
                            if len(v) > 1
                            else [(os.path.basename(v[0][0]),) + v[0][1:]]
                        )
                        for v in value
                    ]
                    ...
                    return [
                        ...
                        for v in value
                        if v and isinstance(v[0], tuple) and v[0][0]
                    ]

                elif (type_ == "LoRA Wt" or type_ == "LoRA MStr") and isinstance(
                    value, list
                ):
            ...
            # Add seed to every entry in the list
            X_value = (
                [v + seed for v in X_value] if "Seeds++ Batch" == X_type else X_value
            )
            Y_value = (
                [v + seed for v in Y_value] if "Seeds++ Batch" == Y_type else Y_value
            )
            ...
            # Special cases
            is_special_case = (
                (X_type == "Refiner On/Off" and Y_type in ["RefineStep", "Steps"])
                or (X_type == "Nothing" and Y_type != "Nothing")
                or (
                    Y_type == "LoRA Batch"
                    and (
                        X_type == "LoRA Wt"
            ...
            flip_xy = (
                y_priority < x_priority and not are_both_conditioners
            ) or is_special_case
            ...
            lora_dict = (
                [
                    ...
                    for t in dict_map.get("LoRA", [])
                ]
                if dict_map.get("LoRA", [])
                else []
            )
            ...
            if ckpt_dict and lora_dict:
                lora_dict = [
                    (lora_stack, ckpt) for ckpt in ckpt_dict for lora_stack in lora_dict
                ]
            ...
            def define_variable(
                ...
                # If var_type is "Positive Prompt S/R", update positive_prompt and generate labels
                    ...
                    if replace_txt != None:
                        ...
                        if positive_prompt[2] is not None:
                            positive_prompt = (
                                positive_prompt[2].replace(search_txt, replace_txt, 1),
                                positive_prompt[1],
                                positive_prompt[2],
                            )
                        else:
                            positive_prompt = (
                                positive_prompt[1].replace(search_txt, replace_txt, 1),
                                positive_prompt[1],
                                positive_prompt[1].replace(search_txt, replace_txt, 1),
                            )
                    else:
                ...
                # If var_type is "Negative Prompt S/R", update negative_prompt and generate labels
                    ...
                    if replace_txt != None:
                        ...
                        if negative_prompt[2] is not None:
                            negative_prompt = (
                                negative_prompt[2].replace(search_txt, replace_txt, 1),
                                negative_prompt[1],
                                negative_prompt[2],
                            )
                        else:
                            negative_prompt = (
                                negative_prompt[1].replace(search_txt, replace_txt, 1),
                                negative_prompt[1],
                                negative_prompt[1].replace(search_txt, replace_txt, 1),
                            )
                    else:
                ...
                # If var_type is "Refiner On/Off", set end_at_step = max steps and generate labels
                    ...
                    if not lora_stack or var_type == "LoRA Stacks":
                        ...
                        lora_stack[0] = tuple(
                            v if v is not None else lora_stack[0][i]
                    ...
                    if var_type == "LoRA" or var_type == "LoRA Stacks":
                        if len(lora_stack) == 1:
                            lora_model_wt = (
                                format(float(lora_model_wt), ".2f")
                                .rstrip("0")
                                .rstrip(".")
                            )
                            lora_clip_wt = (
                                format(float(lora_clip_wt), ".2f")
                                .rstrip("0")
                                .rstrip(".")
                            )
                            lora_filename = lora_filename[
                                : max_label_len - len(f"LoRA: ({lora_model_wt})")
                            ]
                            ...
                            lora_details = [
                                ...
                                for _, lora_model_wt, lora_clip_wt in lora_stack
                            ]
                            non_name_length = sum(
                                len(f"({lora_details[i][0]},{lora_details[i][1]})") + 2
                                for i in range(len(lora_stack))
                            )
                            ...
                            text_elements = [
                                (
                                    f"{lora_filename}({lora_details[i][0]})"
                                    if lora_details[i][0] == lora_details[i][1]
                                    else f"{lora_filename}({lora_details[i][0]},{lora_details[i][1]})"
                                )
                                for i, lora_filename in enumerate(lora_filenames)
                            ]
                        ...
                        lora_model_wt = (
                            format(float(lora_model_wt), ".2f").rstrip("0").rstrip(".")
                        )
                        ...
                        lora_clip_wt = (
                            format(float(lora_clip_wt), ".2f").rstrip("0").rstrip(".")
                        )
                    ...
                    if cnet_stack[0] is None:
                        ...
                        for i, value in enumerate(
                            cnet_stack[1][-3:]
                        ):  # Considering last 3 entries
                            if value is not None:
                                first_cn_entry[i + 2] = (
                                    value  # "+2" to offset for the first 2 entries of the tuple
                                )
                        ...
                        updated_cnet_stack_0 = [updated_first_entry] + list(
                            cnet_stack[0][1:]
                        )
                ...
                def truncate_texts(texts, num_label, max_label_len):
                    truncate_length = max(
                        min(max(len(text) for text in texts), max_label_len), 24
                    )
                ...
                if len(var_label) == num_label and (
                    var_type == "VAE"
                    or var_type == "Checkpoint"
                    or var_type == "Refiner"
                    or "LoRA" in var_type
                ):
            ...
            def define_model(
                ...
                if (
                    X_type == "Checkpoint"
                    and index == 0
                    and Y_type not in ("LoRA", "LoRA Stacks")
                ):
                    if lora_stack is None:
                        model, clip, _ = load_checkpoint(
                            ckpt_name, xyplot_id, cache=cache[1]
                        )
                ...
                # Load LoRA if required
                    ...
                    model, clip = load_lora(
                        lora_stack, ckpt_name, xyplot_id, cache=cache[2]
                    )
                    encode = True
                elif Y_type in (
                    "LoRA",
                    "LoRA Stacks",
                ):  # X_type must be Checkpoint, so cache those as defined
                    model, clip = load_lora(
                        lora_stack,
                        ckpt_name,
                        xyplot_id,
                ...
                if (
                    X_type in encode_refiner_types and index == 0
                ) or Y_type in encode_refiner_types:
                    encode_refiner = True

                # Encode base prompt
                if encode == True:
                    positive, negative, clip = encode_prompts(
                        positive_prompt,
                        negative_prompt,
                        token_normalization,
                        weight_interpretation,
                    ...
                    if cnet_stack:
                        controlnet_conditioning = (
                            TSC_Apply_ControlNet_Stack().apply_cnet_stack(
                ...
                if encode_refiner == True:
                    refiner_positive, refiner_negative, refiner_clip = encode_prompts(
                        positive_prompt,
                        negative_prompt,
                        token_normalization,
                        weight_interpretation,
                ...
                if (X_type == "VAE" and index == 0) or Y_type == "VAE":
                    ...
                    vae = (
                        get_bvae_by_ckpt_name(ckpt_name)
                        if vae_name == "Baked VAE"
            ...
            def process_values(
                ...
                if capsule_result is None:
                    ...
                    image = (
                        ...
                        else vae_decode_latent(vae, samples, vae_decode)
                    )
            ...
            (
                ...
                original_positive,
                original_negative,
                original_refiner_model,
                original_refiner_clip,
                original_refiner_positive,
                original_refiner_negative,
            ) = clone_or_none(
                model,
                clip,
                positive,
            ...
            for X_index, X in enumerate(X_value):
                ...
                if X_type != "Nothing" and Y_type == "Nothing":
                    ...
                    # Generate Results
                    latent_list, image_tensor_list, image_pil_list = process_values(
                        model,
                        refiner_model,
                        add_noise,
                        seed,
            ...
            if cache_models == "False":
                clear_cache_by_exception(
                    xyplot_id, vae_dict=[], ckpt_dict=[], lora_dict=[], refn_dict=[]
                )
            else:
                # Avoid caching models accross both X and Y
                if X_type == "Checkpoint":
            ...
            def print_plot_variables(
                ...
                def get_vae_name(X_type, Y_type, X_value, Y_value, vae_name):
                    if X_type == "VAE":
                        ...
                        vae_name = "\n      ".join(
                            map(
                                lambda y: os.path.splitext(os.path.basename(str(y)))[0],
                                Y_value,
                            )
                        )
                    elif vae_name:
                ...
                def get_checkpoint_name(
                    ...
                    if X_type == primary_type:
                        ...
                        clip_skip = get_clip_skip(
                            X_type, Y_type, X_value, Y_value, clip_skip, mode
                        )
                        if mode == "ckpt":
                            if vae_name:
                                vae_name = get_vae_name(
                                    X_type, Y_type, X_value, Y_value, vae_name
                                )
                    ...
                    ckpt_name = "\n      ".join(
                        [format_name(v, othr_type) for v in ckpt_value]
                    )
                    if mode == "ckpt":
                        return ckpt_name, clip_skip, vae_name
                    else:
                ...
                def get_lora_name(X_type, Y_type, X_value, Y_value, lora_stack=None):
                    ...
                    def get_lora_name_for_weights(lora_path, lora_name):
                        if lora_path:
                            return (
                                os.path.basename(X_value[0][0][0])
                    ...
                    if X_type not in lora_types and Y_type not in lora_types:
                        ...
                        if X_type in lora_types:
                            ...
                            if X_type == "LoRA Batch":
                                ...
                                lora_model_str = (
                                    X_value[0][0][1]
                                    if lora_model_str is None
                                    else lora_model_str
                                )
                                lora_clip_str = (
                                    X_value[0][0][2]
                                    if lora_clip_str is None
                                    else lora_clip_str
                                )
                            elif X_type == "LoRA MStr":
                                lora_name = get_lora_name_for_weights(
                                    X_value[0][0][0], lora_name
                                )
                ...
                def get_lora_sublist_name(lora_type, lora_value):
                    if lora_type in ("LoRA", "LoRA Batch", "LoRA Stacks"):
                        ...
                        for sublist in lora_value:
                            ...
                            for x in sublist:
                                base_name = os.path.splitext(
                                    os.path.basename(str(x[0]))
                                )[0]
                                formatted_str = (
                                    f"{base_name}({round(x[1], 3)},{round(x[2], 3)})"
                                    if lora_type in ("LoRA", "LoRA Stacks")
                                    else f"{base_name}"
                                )
                        ...
                        return ", ".join(
                            [str(round(x[0][1], 3)) for x in lora_value]
                        )  # assuming LoRA Wt uses the second value
                    else:
                        return ""

                # VAE, Checkpoint, Clip Skip, LoRA
                ckpt_name, clip_skip, vae_name = get_checkpoint_name(
                    X_type,
                    Y_type,
                    X_value,
                    Y_value,
                ...
                lora_name, lora_wt, lora_model_str, lora_clip_str = get_lora_name(
                    X_type, Y_type, X_value, Y_value, lora_stack
                )
                refiner_name, refiner_clip_skip = get_checkpoint_name(
                    X_type,
                    Y_type,
                    X_value,
                    Y_value,
                ...
                end_at_step = (
                    ", ".join(map(str, X_value))
                    if X_type in ["EndStep", "RefineStep"]
                    else (
                        ", ".join(map(str, Y_value))
                        if Y_type in ["EndStep", "RefineStep"]
                        else end_at_step
                    )
                )
                ...
                return_with_leftover_noise = (
                    ", ".join(map(str, X_value))
                    if X_type == "ReturnNoise"
                    else (
                        ", ".join(map(str, Y_value))
                        if Y_type == "ReturnNoise"
                ...
                if X_type == "Sampler":
                    ...
                    scheduler = (
                        ", ".join(
                            [
                                str(x[0]) if isinstance(x, tuple) else str(x)
                                for x in X_value
                            ]
                        )
                        if X_type == "Scheduler"
                            ...
                            ", ".join(
                                [
                                    str(y[0]) if isinstance(y, tuple) else str(y)
                                    for y in Y_value
                                ]
                            )
                            if Y_type == "Scheduler"
                ...
                if sampler_type == "sdxl":
                    if refiner_clip_skip == "_":
                        ...
                        print(
                            f"-ascore: {neg_ascore if neg_ascore is not None else ''}"
                        )
                if lora_name:
                ...
                if X_type == "Positive Prompt S/R" or Y_type == "Positive Prompt S/R":
                    positive_prompt = (
                        ", ".join(
                            [
                                str(x[0]) if i == 0 else str(x[1])
                                for i, x in enumerate(X_value)
                            ]
                        )
                        if X_type == "Positive Prompt S/R"
                        else (
                            ", ".join(
                                [
                                    str(y[0]) if i == 0 else str(y[1])
                                    for i, y in enumerate(Y_value)
                                ]
                            )
                            if Y_type == "Positive Prompt S/R"
                ...
                if "ControlNet" in X_type:
                    if "Strength" in X_type:
                        cnet_strength = [
                            str(round(inner_list[0][2], 3))
                            for inner_list in X_value
                            if isinstance(inner_list, list)
                            and inner_list
                            and isinstance(inner_list[0], tuple)
                            and len(inner_list[0]) >= 3
                        ]
                    if "Start%" in X_type:
                        cnet_start_pct = [
                            str(round(inner_list[0][3], 3))
                            for inner_list in X_value
                            if isinstance(inner_list, list)
                            and inner_list
                            and isinstance(inner_list[0], tuple)
            ...
            def adjusted_font_size(text, initial_font_size, i_width):
                ...
                if text_width > (i_width * 0.9):
                    ...
                    new_font_size = int(
                        initial_font_size * (i_width / text_width) * scaling_factor
                    )
                else:
            ...
            if flip_xy == True:
                ...
                image_tensor_list = rearrange_list_A(
                    image_tensor_list, num_cols, num_rows
                )
                latent_list = rearrange_list_A(latent_list, num_cols, num_rows)

            # Extract final image dimensions
            i_height, i_width = (
                image_tensor_list[0].shape[1],
                image_tensor_list[0].shape[2],
            )

            # Print XY Plot Results
            print_plot_variables(
                X_type,
                Y_type,
                X_value,
                Y_value,
            ...
            if Y_label_orientation == "Vertical":
                ...
                border_size_left = min(i_width, i_height) + int(
                    0.2 * abs(i_width - i_height)
                )
            ...
            background = Image.new(
                "RGBA", (int(bg_width), int(bg_height)), color=(255, 255, 255, 255)
            )
                ...
                for col in range(num_cols):
                    ...
                    if row == 0 and X_type != "Nothing":
                        ...
                        font_size = adjusted_font_size(
                            text, initial_font_size, img.width
                        )
                        label_height = int(font_size * 1.5)

                        # Create a white background label image
                        label_bg = Image.new(
                            "RGBA", (img.width, label_height), color=(255, 255, 255, 0)
                        )
                        ...
                        _, _, text_width, text_height = d.textbbox(
                            [0, 0], text, font=font
                        )
                    ...
                    if col == 0 and Y_type != "Nothing":
                        ...
                        if Y_label_orientation == "Vertical":
                            initial_font_size = int(
                                48 * i_width / 512
                            )  # Adjusting this to be same as X_label size
                            font_size = adjusted_font_size(
                                text, initial_font_size, i_width
                            )
                        else:  # Assuming Y_label_orientation is "Horizontal"
                            initial_font_size = int(
                                48 * (border_size_left / Y_label_scale) / 512
                            )  # Adjusting this to be same as X_label size
                            font_size = adjusted_font_size(
                                text,
                                initial_font_size,
                                int(border_size_left / Y_label_scale),
                            )

                        # Create a white background label image
                        label_bg = Image.new(
                            "RGBA",
                            (img.height, int(font_size * 1.2)),
                            color=(255, 255, 255, 0),
                        )
                        d = ImageDraw.Draw(label_bg)
    ...
    try:
        if include_subdirs:
            ...
            batch_files = [
                ...
                and any(f.endswith(ext) for ext in valid_extensions)
            ]
...
class TSC_XYplot:
    ...
    def XYplot(
        ...
        if (
            X_type != "XY_Capsule"
            and (X_type == Y_type)
            and X_type not in ["Positive Prompt S/R", "Negative Prompt S/R"]
        ):
            if X_type != "Nothing":
                print(
                    f"{error('XY Plot Error:')} X and Y input types must be different."
                )
            return (None,)
        ...
        if (X_type in lora_types and Y_type not in lora_types) or (
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "batch_count": (
                    "INT",
                    {"default": XYPLOT_DEF, "min": 0, "max": XYPLOT_LIM},
                ),
            },
        }
...
class TSC_XYplot_Sampler_Scheduler:
    ...
    def INPUT_TYPES(cls):
        ...
        inputs = {
            "required": {
                ...
                "input_count": (
                    "INT",
                    {"default": XYPLOT_DEF, "min": 0, "max": XYPLOT_LIM, "step": 1},
                ),
            }
        }
    ...
    def xy_value(self, target_parameter, input_count, **kwargs):
        if target_parameter == "scheduler":
            xy_type = "Scheduler"
            schedulers = [
                kwargs.get(f"scheduler_{i}") for i in range(1, input_count + 1)
            ]
            ...
            xy_value = [
                (sampler, scheduler if scheduler != "None" else None)
    ...
    def INPUT_TYPES(cls):
        ...
        inputs = {
            "required": {
                ...
                "batch_path": (
                    "STRING",
                    {"default": xy_batch_default_path, "multiline": False},
                ),
                "subdirectories": ("BOOLEAN", {"default": False}),
                "batch_sort": (["ascending", "descending"],),
                "batch_max": (
                    "INT",
                    {"default": -1, "min": -1, "max": XYPLOT_LIM, "step": 1},
                ),
                "vae_count": (
                    "INT",
                    {"default": XYPLOT_DEF, "min": 0, "max": XYPLOT_LIM, "step": 1},
    ...
    def xy_value(
        ...
        **kwargs,
        ...
        if "Batch" not in input_mode:
            ...
            try:
                vaes = get_batch_files(
                    batch_path, VAE_EXTENSIONS, include_subdirs=subdirectories
                )
    ...
    def INPUT_TYPES(cls):
        inputs = {
            "required": {
                ...
                "replace_count": (
                    "INT",
                    {"default": XYPLOT_DEF, "min": 0, "max": XYPLOT_LIM - 1},
                ),
            }
        }
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "refine_at_percent": (
                    "FLOAT",
                    {"default": 0.80, "min": 0.00, "max": 1.00, "step": 0.01},
                )
            },
        }
    ...
    def xy_value(
        ...
        **kwargs,
        ...
        if "Batch" not in input_mode:
            # Extract values from kwargs
            checkpoints = [
                kwargs.get(f"ckpt_name_{i}") for i in range(1, ckpt_count + 1)
            ]
            clip_skips = [
                kwargs.get(f"clip_skip_{i}") for i in range(1, ckpt_count + 1)
            ]
            ...
            xy_value = [
                (checkpoint, clip_skip, vae)
                for checkpoint, clip_skip, vae in zip(checkpoints, clip_skips, vaes)
                if checkpoint != "None"
            ]
        else:
            if batch_max == 0:
            ...
            try:
                ckpts = get_batch_files(
                    batch_path, CKPT_EXTENSIONS, include_subdirs=subdirectories
                )
...
class TSC_XYplot_LoRA_Batch:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "model_strength": (
                    "FLOAT",
                    {"default": 1.0, "min": -10.00, "max": 10.0, "step": 0.01},
                ),
                "clip_strength": (
                    "FLOAT",
                    {"default": 1.0, "min": -10.0, "max": 10.0, "step": 0.01},
    ...
    def xy_value(
        ...
        loras = get_batch_files(
            batch_path, LORA_EXTENSIONS, include_subdirs=subdirectories
        )
        ...
        xy_value = [
            [(lora, model_strength, clip_strength)] + (lora_stack if lora_stack else [])
            for lora in loras
        ]
    ...
    def xy_value(
        ...
        **kwargs,
        ...
        if "Batch" not in input_mode:
            ...
            loras = [kwargs.get(f"lora_name_{i}") for i in range(1, lora_count + 1)]
            model_strs = [
                kwargs.get(f"model_str_{i}", model_strength)
                for i in range(1, lora_count + 1)
            ]
            clip_strs = [
                kwargs.get(f"clip_str_{i}", clip_strength)
                for i in range(1, lora_count + 1)
            ]
            ...
            xy_value = [
                [(lora, model_str, clip_str)] + lora_stack
                for lora, model_str, clip_str in zip(loras, model_strs, clip_strs)
                if lora != "None"
            ]
            ...
            try:
                result = self.lora_batch.xy_value(
                    batch_path,
                    subdirectories,
                    batch_sort,
                    model_strength,
...
class TSC_XYplot_LoRA_Plot:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "X_first_value": (
                    "FLOAT",
                    {"default": 0.0, "min": -10.00, "max": 10.0, "step": 0.01},
                ),
                "X_last_value": (
                    "FLOAT",
                    {"default": 1.0, "min": -10.00, "max": 10.0, "step": 0.01},
    ...
    def xy_value(
        ...
        if "X: LoRA Batch" in input_mode:
            try:
                x_value = self.lora_batch.xy_value(
                    X_batch_path,
                    X_subdirectories,
                    X_batch_sort,
                    model_strength,
...
class TSC_XYplot_Control_Net_Strength:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "first_strength": (
                    "FLOAT",
                    {"default": 0.0, "min": 0.00, "max": 10.0, "step": 0.01},
                ),
                "last_strength": (
                    "FLOAT",
                    {"default": 1.0, "min": 0.00, "max": 10.0, "step": 0.01},
                ),
                "start_percent": (
                    "FLOAT",
                    {"default": 0.0, "min": 0.00, "max": 1.0, "step": 0.01},
                ),
                "end_percent": (
                    "FLOAT",
                    {"default": 1.0, "min": 0.00, "max": 1.0, "step": 0.01},
                ),
            },
            "optional": {"cnet_stack": ("CONTROL_NET_STACK",)},
        }
    ...
    def xy_value(
        ...
        strength_increment = (
            (last_strength - first_strength) / (batch_count - 1)
            if batch_count > 1
            else 0
        )
        ...
        xy_value.append(
            [(control_net, image, first_strength, start_percent, end_percent)]
        )
            ...
            xy_value.append(
                [
                    (
                        control_net,
                        image,
                        first_strength + i * strength_increment,
                        start_percent,
                        end_percent,
                    )
                ]
        ...
        if batch_count > 1:
            xy_value.append(
                [(control_net, image, last_strength, start_percent, end_percent)]
            )
...
class TSC_XYplot_Control_Net_Start:
    ...
    def xy_value(
        ...
        percent_increment = (
            (last_start_percent - first_start_percent) / (batch_count - 1)
            if batch_count > 1
            else 0
        )
        ...
        xy_value.append(
            [(control_net, image, strength, first_start_percent, end_percent)]
        )
            ...
            xy_value.append(
                [
                    (
                        ...
                        image,
                        strength,
                        first_start_percent + i * percent_increment,
                        end_percent,
                    )
                ]
            )
        ...
        if batch_count > 1:
            xy_value.append(
                [(control_net, image, strength, last_start_percent, end_percent)]
            )
...
class TSC_XYplot_Control_Net_End:
    ...
    def xy_value(
        ...
        percent_increment = (
            (last_end_percent - first_end_percent) / (batch_count - 1)
            if batch_count > 1
            else 0
        )
        ...
        xy_value.append(
            [(control_net, image, strength, start_percent, first_end_percent)]
        )
        ...
        if batch_count > 1:
            xy_value.append(
                [(control_net, image, strength, start_percent, last_end_percent)]
            )
...
class TSC_XYplot_Control_Net(
    TSC_XYplot_Control_Net_Strength,
    TSC_XYplot_Control_Net_Start,
    TSC_XYplot_Control_Net_End,
):
    ...
    def xy_value(
        ...
        if target_parameter == "strength":
            return TSC_XYplot_Control_Net_Strength.xy_value(
                self,
                control_net,
                image,
                batch_count,
            ...
            return TSC_XYplot_Control_Net_Start.xy_value(
                self,
                control_net,
                image,
                batch_count,
            ...
            return TSC_XYplot_Control_Net_End.xy_value(
                self,
                control_net,
                image,
                batch_count,
...
class TSC_XYplot_Control_Net_Plot:
    ...
    def get_value(
        ...
        increment = (
            (last_value - first_value) / (batch_count - 1) if batch_count > 1 else 0
        )
        ...
        if axis == "Strength":
            values.append(
                [(control_net, image, first_value, start_percent, end_percent)]
            )
        elif axis == "Start%":
        ...
        if batch_count > 1:
            if axis == "Strength":
                values.append(
                    [(control_net, image, last_value, start_percent, end_percent)]
                )
            elif axis == "Start%":
                values.append([(control_net, image, strength, last_value, end_percent)])
            elif axis == "End%":
                values.append(
                    [(control_net, image, strength, start_percent, last_value)]
                )
        ...
        return {
            "required": {
                "notes": (
                    "STRING",
                    {
                        "default": f"_____________SYNTAX_____________\n{cls.syntax}\n\n"
    ...
    def xy_value(self, plot_type, plot_value):
        ...
        bounds = {
            ...
            "LoRA": {
                ...
                "model_str": {"min": -10, "max": 10},
                "clip_str": {"min": -10, "max": 10},
            },
        }
        ...
        def validate_value(value, value_type, bounds):
            ...
            # Sampler
                ...
                if isinstance(value, tuple):
                    if len(value) >= 2:
                        value = value[
                            :2
                        ]  # Slice the value tuple to keep only the first two elements
                        sampler, scheduler = value
                        scheduler = (
                            scheduler.lower()
                        )  # Convert the scheduler name to lowercase
                ...
                if isinstance(value, tuple):
                    if len(value) >= 2:
                        ...
                        if checkpoint not in bounds["Checkpoint"]["options"]:
                            ...
                            checkpoint = None
                        if (
                            clip_skip < bounds["Clip Skip"]["min"]
            ...
            # LoRA
                ...
                if isinstance(value, tuple):
                    lora_name, model_str, clip_str = (value + (1.0, 1.0))[
                        :3
                    ]  # Defaults model_str and clip_str to 1 if not provided
                    ...
                    if (
                        model_str < bounds["LoRA"]["model_str"]["min"]
                    ...
                    if (
                        clip_str < bounds["LoRA"]["clip_str"]["min"]
        ...
        if plot_type in {"Positive Prompt S/R", "Negative Prompt S/R"}:
            if plot_value[0] == "":
                ...
                plot_value = [
                    (plot_value[0], None) if i == 0 else (plot_value[0], x)
                    for i, x in enumerate(plot_value)
                ]
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "rescale_factor": (
                    "FLOAT",
                    {"default": 1, "min": 0.01, "max": 16.0, "step": 0.01},
                ),
                "width": (
                    "INT",
    ...
    def apply_overlay_image(
        ...
        if overlay_resize != "None":
            ...
            if overlay_resize == "Fit":
                ...
                overlay_image_size = tuple(
                    round(dimension * ratio) for dimension in overlay_image_size
                )
            elif overlay_resize == "Resize by rescale_factor":
                overlay_image_size = tuple(
                    int(dimension * rescale_factor) for dimension in overlay_image_size
                )
            elif overlay_resize == "Resize to width & heigth":
                overlay_image_size = (size[0], size[1])

            samples = overlay_image.movedim(-1, 1)
            overlay_image = comfy.utils.common_upscale(
                samples,
                overlay_image_size[0],
                overlay_image_size[1],
                resize_method,
                False,
            )
...
class TSC_Noise_Control_Script:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "weight": (
                    "FLOAT",
                    {"default": 0.015, "min": 0, "max": 1, "step": 0.001},
                ),
            },
    ...
    def noise_control(
        self, rng_source, cfg_denoiser, add_seed_noise, seed, weight, script=None
    ):
        script = script or {}
...
if os.path.exists(os.path.join(custom_nodes_dir, "comfyui_controlnet_aux")):
    ...
    try:
        with suppress_output():
            AIO_Preprocessor = getattr(
                import_module("comfyui_controlnet_aux.__init__"), "AIO_Preprocessor"
            )
...
class TSC_HighRes_Fix:
    ...
    city96_upscale_methods = [
        "city96." + ver
        for ver in city96_latent_upscaler.LatentUpscaler.INPUT_TYPES()["required"][
            "latent_ver"
        ][0]
    ]
    city96_scalings_raw = city96_latent_upscaler.LatentUpscaler.INPUT_TYPES()[
        "required"
    ]["scale_factor"][0]
    ...
    ttl_nn_upscale_methods = [
        "ttl_nn." + ver
        for ver in ttl_nn_latent_upscaler.NNLatentUpscale.INPUT_TYPES()["required"][
            "version"
        ][0]
    ]

    latent_upscalers = (
        default_latent_upscalers + city96_upscale_methods + ttl_nn_upscale_methods
    )
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                "upscale_type": (["latent", "pixel", "both"],),
                "hires_ckpt_name": (
                    ["(use same)"] + folder_paths.get_filename_list("checkpoints"),
                ),
                ...
                "denoise": (
                    "FLOAT",
                    {"default": 0.56, "min": 0.00, "max": 1.00, "step": 0.01},
                ),
    ...
    def hires_fix_script(
        ...
        if iterations > 0 and upscale_by > 0:
            if upscale_type == "latent":
                ...
                if latent_upscaler in self.city96_upscale_methods:
                    ...
                    nearest_scaling = min(
                        self.city96_scalings_float, key=lambda x: abs(x - upscale_by)
                    )

                    # Retrieve the index of the nearest scaling
                    nearest_scaling_index = self.city96_scalings_float.index(
                        nearest_scaling
                    )

                    # Use the index to get the raw string representation
                    nearest_scaling_raw = self.city96_scalings_raw[
                        nearest_scaling_index
                    ]
                ...
                if hires_ckpt_name == "(use same)":
                    ...
                    latent_upscale_model, _, _ = load_checkpoint(
                        hires_ckpt_name,
                        my_unique_id,
                        output_vae=False,
                        cache=1,
        ...
        control_net = (
            ControlNetLoader().load_controlnet(control_net_name)[0]
            if use_controlnet is True
            else None
        )
...
class TSC_Tiled_Upscaler:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "tile_size": (
                    "INT",
                    {"default": 512, "min": 256, "max": MAX_RESOLUTION, "step": 64},
                ),
                "tiling_strategy": (
    ...
    def tiled_sampling(
        ...
        if tiling_strategy != "none":
            script = script or {}
            tile_controlnet = (
                ControlNetLoader().load_controlnet(tile_controlnet)[0]
                if use_controlnet
                else None
            )
...
def unique_by(s):
    ...
    seen = set()
    r = [k for k in s if k not in seen and not seen.add(k)]
    return r
...
try:
    ...
    class TSC_EvaluateInts:
        ...
        def INPUT_TYPES(cls):
            return {
                "required": {
                    "python_expression": (
                        "STRING",
                        {"default": "((a + b) - c) / 2", "multiline": False},
                    ),
        ...
        def evaluate(self, python_expression, print_to_console, a=0, b=0, c=0):
            ...
            result = simpleeval.simple_eval(
                python_expression, names={"a": a, "b": b, "c": c}
            )
            ...
            if print_to_console == "True":
                ...
                print(
                    f"{python_expression} = \033[92m INT: "
                    + str(int_result)
                    + " , FLOAT: "
                    + str(float_result)
                    + ", STRING: "
    ...
    class TSC_EvalExamples:
        ...
        def INPUT_TYPES(cls):
            ...
            return {
                "required": {
                    "models_text": ("STRING", {"default": examples, "multiline": True}),
                },
            }
